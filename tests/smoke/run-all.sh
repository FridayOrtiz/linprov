...
fail=0
for script in basic.sh inode_storage.sh xattr_fallback.sh and_or.sh deep_paths.sh propagate.sh scripts.sh hot_reload.sh allow.sh subscribe.sh overflow.sh soak.sh; do
    echo
    echo "######## $script ########"
    if ! "$here/$script"; then
